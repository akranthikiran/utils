...
/**
 * Address1 is test entity that is suppose to get deleted with parent employee
 */
@Table("ADDRESS1")
@ForeignConstraints({
	@ForeignConstraint(
			...
			conditions = {@MappingCondition(field="parentType", value = Address1.PARENT_TYPE_EMPLOYEE)},
			foreignEntity = Employee.class,
			message = "Invalid parent id specified ${parentId}",
			deleteCascade = DeleteCascade.DELETE_WITH_PARENT
	)
})
public class Address1
{
	...
	public Address1(String id, String address, String parentId, String parentType)
	{
		this.id = id;
